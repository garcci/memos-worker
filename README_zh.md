...
# Memos Worker：基于 Cloudflare 的笔记与知识库
...
**Memos Worker** 是一个功能强大、性能卓越的无服务器笔记和知识管理应用。完全基于 Cloudflare 生态（Workers, Pages, D1, KV）构建，为您提供一个私有、低成本且可永久拥有的笔记解决方案。

## ✨ 特性一览

-   **✍️ 全功能 Markdown 支持**：支持实时预览、分屏编辑、富文本智能粘贴转 Markdown。
-   **🎛️ 灵活的视图与工作流**：通过归档、收藏、置顶等方式管理笔记。支持列表、瀑布流、按日期分组等多种视图。
-   **🔗 公开分享**：为任意单条笔记生成唯一的、可公开访问的链接，支持设置过期时间。
-   **🤖 Telegram 集成**：通过 Telegram 机器人随时随地记录文字、图片、视频和文件，支持代理模式节省存储空间。
-   **📚 强大的组织能力**：自动标签、全文搜索、时间线、日历和贡献热力图。
-   **📃 知识库 (Docs)**：独立的、树状结构的文档中心，适合构建有组织的知识体系。
-   **🎨 高度可定制**：明暗主题、自定义主色调、背景图片、毛玻璃效果，以及精细的布局和功能可见性调整。
-   **🚀 高性能与低成本**：由 Cloudflare 全球网络提供闪电般的响应速度，在免费额度下几乎零成本运行。
...
你需要在 Cloudflare Dashboard 中手动创建好项目所需的 D1 数据库和 KV 命名空间。
